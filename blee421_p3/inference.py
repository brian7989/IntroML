...
class ExactInference(InferenceModule):
    ...
    def observe(self, observation, gameState):
        ...
        if noisyDistance is None:
            ...
            for pos in self.legalPositions:
                allPossible[pos] = emissionModel[util.manhattanDistance(pos, pacmanPosition)] * self.beliefs.get(pos)
    ...
    def elapseTime(self, gameState):
        ...
        *** GORY DETAIL AHEAD ***
        ...
        for pos in self.legalPositions:
            for item in self.getPositionDistribution(self.setGhostPosition(gameState, pos)).items():
                allPossible[item[0]] += self.beliefs[pos] * item[1]
...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState):
        ...
        """
        "*** YOUR CODE HERE ***"
        # use modular operations to evenly distribute
        self.particles = [self.legalPositions[x % len(self.legalPositions)] for x in range(self.numParticles)]
    ...
    def observe(self, observation, gameState):
        ...
        if noisyDistance is not None:
            for p in self.particles:
                bd[p] += emissionModel[util.manhattanDistance(pacmanPosition, p)]
            if sum(bd.values()):
                for i in range(self.numParticles):
                    self.particles[i] = util.sample(bd)
            else:
                self.initializeUniformly(gameState)
        else:
            jailPos = [self.getJailPosition() for _ in range(self.numParticles)]
            # print(jailPos)
    ...
    def getBeliefDistribution(self):
        ...
        "*** YOUR CODE HERE ***"
        bd = util.Counter()
        bd.incrementAll(self.particles, 1.0); bd.normalize()
        return bd
...
class JointParticleFilter:
    ...
    def initializeParticles(self):
        ...
        """
        "*** YOUR CODE HERE ***"
        lenP = len(self.legalPositions)**2
        p = random.sample(list(itertools.product(self.legalPositions, repeat = self.numGhosts)), lenP)
        self.particles = [p[i % lenP] for i in range(self.numParticles)]
        ...
        for p in self.particles:
            ...
            for i in range(self.numGhosts):
                if noisyDistances[i] is not None:
                    prob *= emissionModels[i][util.manhattanDistance(p[i], pacmanPosition)]; continue
                p = self.getParticleWithGhostInJail(p, i)
            w[p] += prob
        if w.totalCount():
            tmp = []; w.normalize()
            for k in w.sortedKeys():
                [tmp.append(k) for _ in range(int(w[k] * self.numParticles)) if len(tmp) < self.numParticles]
            self.particles = tmp; return
        self.initializeParticles()
    ...
    def getBeliefDistribution(self):
        "*** YOUR CODE HERE ***"
        bd = util.Counter()
        bd.incrementAll(self.particles, 1.0)
        bd.normalize()
        return bd
