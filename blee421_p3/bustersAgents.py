...
class GreedyBustersAgent(BustersAgent):
    ...
    def chooseAction(self, gameState):
        ...
        livingGhostPositionDistributions = [beliefs for i,beliefs
                                            ...
                                            if livingGhosts[i+1]]
        "*** YOUR CODE HERE ***"
        # Finding position associated with the maximum prob
        posProbPair = [None, 0.0]
        for posDist in livingGhostPositionDistributions:
            for item in posDist.items():
                posProbPair = item if item[1] > posProbPair[1] else posProbPair
        distActPair = [self.distancer.getDistance(Actions.getSuccessor(pacmanPosition, legal[0]), posProbPair[0]), legal[0]]

        # Finding legal action associated with the minimum distance
        for legalAct in legal:
            potentialMinD = self.distancer.getDistance(Actions.getSuccessor(pacmanPosition, legalAct), posProbPair[0])
            distActPair = [potentialMinD, legalAct] if potentialMinD < distActPair[0] else distActPair
        return distActPair[1]
