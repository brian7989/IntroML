...
class Tree:
  ...
  """
    ...
    def classifyHelper(self, cd, node):
        return node.value if node.isleaf else self.classifyHelper(cd, node.children[cd[node.attr]])
...
def getPertinentExamples(examples, attrName, attrValue):
    ...
    """
    return [ex for ex in examples if ex[attrName] == attrValue]
...
def getClassCounts(examples, className):
    ...
    classCounts = {}
    for ex in examples:
        if not ex[className] in classCounts:
            classCounts[ex[className]] = 0
...
def getAttributeCounts(examples, attrName, attrValues, className):
    ...
    """
    return {v: getClassCounts(getPertinentExamples(examples, attrName, v), className) for v in attrValues}
...
def setEntropy(classCounts):
    ...
    for i in classCounts:
        p = float(i) / sum(classCounts)
        entropy -= log(p ** p, 2)
    return entropy


def calculateValue(ex, attrName, attrVal, className, multiplier):
    return sum([sum(v.values()) / float(len(ex)) * multiplier(v.values())
                for v in getAttributeCounts(ex, attrName, attrVal, className).values()])
...
def remainder(examples, attrName, attrValues, className):
    ...
    """
    return calculateValue(examples, attrName, attrValues, className, setEntropy)
...
def infoGain(examples, attrName, attrValues, className):
    ...
    """
    e = setEntropy(getClassCounts(examples, className).values())
...
def giniIndex(classCounts):
    ...
    """
    return 1 - sum([(float(c) / sum(classCounts)) ** 2 for c in classCounts])
...
def giniGain(examples, attrName, attrValues, className):
    ...
    """
    giniD = calculateValue(examples, attrName, attrValues, className, giniIndex)
    return 1 / giniD if giniD else sys.maxint
...
def makeSubtrees(remainingAttributes, examples, attributeValues, className, defaultLabel, setScoreFunc, gainFunc):
    ...
    # Non-trivial case
    attr = max(remainingAttributes, key=lambda x: gainFunc(examples, x, attributeValues[x], className))
    root = Node(attr)

    for v in attributeValues[attr]:
        root.children[v] = makeSubtrees([a for a in remainingAttributes if a != attr],
                                        getPertinentExamples(examples, attr, v), attributeValues, className,
...
def makePrunedSubtrees(remainingAttributes, examples, attributeValues, className, defaultLabel, setScoreFunc, gainFunc,
    ...
    # Non-trivial case
    attr = max(remainingAttributes, key=lambda x: gainFunc(examples, x, attributeValues[x], className))
    attrCount = getAttributeCounts(examples, attr, attributeValues[attr], className)
    classCount, chiSq, root = getClassCounts(examples, className), 0, Node(attr)

    for i in attributeValues[attr]:
        for j in classCount:
            if j not in attrCount[i]: attrCount[i][j] = 0
            fExp = sum(attrCount[i].values()) * float(classCount[j]) / sum(classCount.values())
    ...
    for v in attributeValues[attr]:
        root.children[v] = makePrunedSubtrees([a for a in remainingAttributes if a != attr],
                                              getPertinentExamples(examples, attr, v), attributeValues, className,
                                              getMostCommonClass(examples, className), setScoreFunc, gainFunc, q)
    return root
